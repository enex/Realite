...
function ProfileEventList({ mode, events }: ProfileEventListProps) {
  if (!events.length) {
    if (mode === "owner") {
      return <p className="text-sm text-muted-foreground">Noch keine kommenden Events auf deinem Profil.</p>;
    }

    if (mode === "matched") {
      return <p className="text-sm text-muted-foreground">Aktuell gibt es keine gematchten, kommenden Events.</p>;
    }

    return <p className="text-sm text-muted-foreground">Aktuell gibt es keine öffentlichen #alle-Events.</p>;
  }
      ...
      {events.map((event) => {
        ...
        return (
          <article key={event.id} className="overflow-hidden rounded-xl border border-border bg-card">
            <div className="flex">
              {coverUrl ? (
                <a href={`/e/${shortenUUID(event.id)}`} className="relative block h-20 w-24 shrink-0 bg-muted sm:h-24 sm:w-28">
              ...
              <div className="min-w-0 flex-1 p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <a
                      ...
                      className="font-semibold text-foreground underline decoration-border underline-offset-2 hover:decoration-teal-500"
                    >
                      {event.title}
                    </a>
                    ...
                    {event.location ? <p className="mt-1 text-xs text-muted-foreground">Ort: {event.location}</p> : null}
                    {event.groupName ? <p className="mt-1 text-xs text-muted-foreground">Gruppe: {event.groupName}</p> : null}
                  </div>
                ...
                {event.tags.length ? (
                  <div className="mt-3 flex flex-wrap gap-2">
                    {event.tags.map((tag) => (
                      <span key={`${event.id}-${tag}`} className="rounded-full bg-muted px-2 py-1 text-[11px] text-muted-foreground">
                        {tag}
                      </span>
                    ))}
                  </div>
