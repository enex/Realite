...
export function SinglesProfileImageField({
  ...
  const handlePick = useCallback(
    (fileList: FileList | null) => {
      ...
      const looksImage =
        file.type.startsWith("image/") || isHeicLike(file);
      if (!looksImage) {
  ...
  const confirmCrop = useCallback(async () => {
    ...
    try {
      ...
      onError?.(
        e instanceof Error ? e.message : "Bild konnte nicht verarbeitet werden.",
      );
    } finally {
      setExporting(false);
    }
    ...
    <div className="grid gap-2 text-sm">
      ...
      <div className="flex flex-wrap items-start gap-4">
        ...
        <label
          ...
          className={`flex min-h-[7.5rem] min-w-[min(100%,18rem)] flex-1 cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-teal-600/40 bg-teal-50/50 px-4 py-5 text-center transition hover:border-teal-700/60 hover:bg-teal-50 dark:border-teal-500/35 dark:bg-teal-950/20 dark:hover:border-teal-400/50 ${
            disabled || busy || exporting ? "pointer-events-none opacity-50" : ""
          }`}
        >
          <UploadSimple className="h-8 w-8 text-teal-700 dark:text-teal-400" weight="duotone" />
          <span className="font-semibold text-foreground">
            Foto wählen oder hierher ziehen
          </span>
          <span className="max-w-sm text-xs leading-5 text-muted-foreground">
            Damit andere dich vor Ort wiedererkennen, am liebsten ein klares Bild
            von vorne — am Handy reicht oft ein kurzes Selfie. Du kannst auch ein
            vorhandenes Foto nutzen und den Ausschnitt gleich noch anpassen.
          </span>
          <input
            id={inputId}
            type="file"
      ...
      {busy ? (
        ...
        <p className="text-xs text-muted-foreground">
          Nach dem Hochladen Profil speichern, damit andere das Bild sehen.
        </p>
      )}
        ...
        <DialogContent className="gap-0 p-0 md:max-h-[90vh]" showCloseButton>
          <div className="border-b border-border px-4 pb-3 pt-4 pr-14 md:px-5">
            ...
            <DialogDescription className="mt-1">
              Verschiebe und zoome das Bild so, dass dein Gesicht gut erkennbar im
              Kreis liegt — damit andere dich vor Ort wiedererkennen.
            </DialogDescription>
          </div>
          {imageSrc ? (
