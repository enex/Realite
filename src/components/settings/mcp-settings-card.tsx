...
function UrlRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-slate-200 bg-white px-3 py-3">
      ...
      <code className="mt-2 block break-all text-xs text-slate-800">{value}</code>
    </div>
  );
}
    ...
    <section className="mt-6 rounded-2xl border border-teal-200 bg-teal-50/60 p-6 shadow-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <h2 className="mt-1 text-lg font-semibold text-slate-900">Realite mit deinem AI-Tool verbinden</h2>
        ...
        <a
          ...
          className="inline-flex items-center justify-center rounded-full border border-teal-300 bg-white px-4 py-2 text-sm font-semibold text-teal-800 transition hover:bg-teal-100"
        >
          Anleitung öffnen
        </a>
      </div>
