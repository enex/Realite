...
type SettingsPayload = {
  ...
  criteria: {
    positiveCriteria: Array<{
      key: string;
      label: string;
      weight: number;
      votes: number;
    }>;
    negativeCriteria: Array<{
      key: string;
      label: string;
      weight: number;
      votes: number;
...
async function fetchSettings(): Promise<SettingsPayload> {
  ...
  if (!response.ok) {
    throw new Error(
      payload.error ?? "Einstellungen konnten nicht geladen werden",
    );
  }
  return payload;
}
...
export function SettingsPage({
  ...
  const settingsMessaging = getSuggestionSettingsMessaging(
    data.calendarConnectionState,
  );
  ...
  const [suggestionForm, setSuggestionForm] = useState<SuggestionSettingsForm>(
  ...
  async function saveSuggestionSettings(event: React.FormEvent) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          payload.error ?? "Einstellungen konnten nicht gespeichert werden",
        );
      }
  ...
  async function uploadProfileImage(file: File | undefined) {
    ...
    try {
      ...
      if (!uploadResponse.ok || !uploadPayload.imageUrl) {
        throw new Error(
          uploadPayload.error ?? "Profilbild konnte nicht hochgeladen werden",
        );
      }
      ...
      if (!saveResponse.ok || !savePayload.profile) {
        throw new Error(
          savePayload.error ?? "Profilbild konnte nicht gespeichert werden",
        );
      }
      ...
      toast.error(
        imageError instanceof Error ? imageError.message : "Unbekannter Fehler",
      );
    } finally {
  ...
  async function deleteAccount() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          payload.error ?? "Account konnte nicht gelöscht werden",
        );
      }
      ...
      <main className="mx-auto w-full max-w-4xl px-4 py-6 sm:px-6 lg:px-8">
        <header className={pageShellClassName}>
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className={managementPage.eyebrowClassName}>
                Profil & Einstellungen
              </p>
              <h1 className={pageTitleClassName}>{userName}</h1>
              <p className={pageMetaClassName}>
                {isAnonymous ? "Temporärer Gastzugang" : userEmail}
              </p>
          ...
          <div className="mt-4">
            <SinglesProfileImageField
              ...
              description="Am besten ein klares Bild von dir — ein Selfie reicht. Du kannst den Ausschnitt direkt anpassen."
              helpText="Wird direkt gespeichert, kein weiterer Schritt nötig."
            />
          </div>
        </header>

        {loading ? (
          <p className="mt-6 text-muted-foreground">Lade Einstellungen...</p>
        ) : null}
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-teal-800">
                  Verbinde einen echten Login, damit du diesen Zugang später
        ...
        <SuggestionSettingsCard
          ...
          autoInsertedSuggestionCount={
            data.suggestionStats.autoInsertedSuggestionCount
          }
